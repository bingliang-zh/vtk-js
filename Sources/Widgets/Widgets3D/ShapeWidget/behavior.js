...
export default function widgetBehavior(publicAPI, model) {
  ...
  publicAPI.makeSquareFromPoints = (point1, point2) => {
    ...
    let sh = vec3.dot(d, horizontal);
    let sv = vec3.dot(d, vertical);
    const absSh = Math.abs(sh);
    const absSv = Math.abs(sv);

    if (absSh < EPSILON) {
      sh = sv;
    } else if (absSv < EPSILON) {
      sv = sh;
    } else if (absSh > absSv) {
      sv = (sv / absSv) * absSh;
    } else {
    ...
    return [
      point1[0] + sh * horizontal[0] + sv * vertical[0],
      point1[1] + sh * horizontal[1] + sv * vertical[1],
      point1[2] + sh * horizontal[2] + sv * vertical[2],
    ];
  };
  ...
  const getCornersFromRadius = (center, pointOnCircle) => {
    ...
    const point1 = [
      center[0] + (vertical[0] - right[0]) * radius,
      center[1] + (vertical[1] - right[1]) * radius,
      center[2] + (vertical[2] - right[2]) * radius,
    ];
    const point2 = [
      center[0] + (right[0] - vertical[0]) * radius,
      center[1] + (right[1] - vertical[1]) * radius,
      center[2] + (right[2] - vertical[2]) * radius,
    ];
  ...
  */
  ...
  publicAPI.handleMouseMove = (callData) => {
    ...
    const vertical = model.camera.getViewUp();
    const horizontal = [0, 0, 0];
    vec3.cross(horizontal, vertical, normal);
    model.shapeHandle.setUp(vertical);
    model.shapeHandle.setRight(horizontal);
